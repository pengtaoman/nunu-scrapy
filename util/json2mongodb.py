...
def queryjsonfilepage(file_name, frompage, topage, searchkey, **kwargs):
    with open(file_name, 'r', encoding='utf-8') as f:
        obj = ijson.items(f, 'item')
        cnt = 0
        while True:
                ...
                for k, v in kwargs.items():
                    if v == '':
                        if  k != 'name':
                            ...
                            if objec[k].find(searchkey) != -1:
                                ische = ische + 1
                    elif isinstance(objec[k], list):
                        if len([1 for ele in objec[k] if ele.find(v) != -1]) > 0:
                            ische = ische + 1
                    elif k == 'rate':
                        if float(objec[k]) > float(v):
...
def dosearch(obj, skey):
    print('##############' + skey)
    if obj['name'].find(skey) != -1:
        return obj
    else:
        return None
